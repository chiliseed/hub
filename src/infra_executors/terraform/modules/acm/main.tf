...
//  validation_domains = [for k, v in aws_acm_certificate.ssl[0].domain_validation_options : tomap(v) if contains(local.distinct_domain_names, v.domain_name)]
//}
//
...
//  count = var.create_certificate && var.validation_method == "DNS" && var.validate_certificate ? length(local.distinct_domain_names) : 0
  ...
  records = [
    aws_acm_certificate.ssl.domain_validation_options[0].resource_record_value,
  ]
